...
class InterfazMain(tk.Tk, funcRegistro):
    ...
    def login(self):
        usuario = self.username.get()  # Obtén el nombre de usuario del campo de entrada
        contraseña = self.password.get()  # Obtén la contraseña del campo de entrada
        operador = self.verificar_operador(usuario, contraseña)
        if operador:
            # Si la verificación es exitosa, enviar los datos del operador a la API
            data = {
            ...
            try:
                response = requests.post("https://realtime.bucr.digital/api/operator", json=data)
